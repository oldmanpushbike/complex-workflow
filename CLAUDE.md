...
8. 人类审批后（增量修改可跳过此步）：将提案发送给 Codex 进行 grill-me 挑战（技术风险审查——Agent 领地，非设计决策）。通过 CLI、MCP 或交接文件。
...
9. 在验证日志中记录降级情况。
10. **若 Codex 恢复可用：** 遵循 `workflow/resume-protocol.md` 的回归检测（第二章）和模式切换协议（第三章）。所有在 single-agent 下通过的制品将自动排入回归验证队列。
